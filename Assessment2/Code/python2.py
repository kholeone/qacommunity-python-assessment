...
def one(input):
	three_char_string = ""
	for x in input:
		three_char_string = three_char_string + x
		three_char_string = three_char_string + x
		three_char_string = three_char_string + x
	return three_char_string
...
def three(a):
	...
	number_list.append(number_input*2)
	number_list.append(number_input*3)
	number_list.append(number_input*4)
	number = int(number_list[0]) + int(number_list[1]) + int(number_list[2]) + int(number_list[3])
	return number
...
def four(input1, input2):
	...
	for x in range(len(input1)):
		character_list.insert(zipper,input1[x])
		zipper += 1
		character_list.insert(zipper,input2[x])
		zipper += 1
	for y in character_list:
		merged_string = merged_string + y
...
def seven(a, b, c):
	...
	difference_one = number_list[2] - number_list[1]
	difference_two = number_list[1] - number_list[0]
...
def eight(input,  a):
	string_length = int((len(input) - a)/2)
	string_length2 = int(string_length + a)
	string_start = input[0:string_length]
	string_end = input[string_length2:]
	string_result = string_start + string_end
